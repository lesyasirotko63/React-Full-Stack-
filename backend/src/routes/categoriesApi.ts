...
import { newCategoriesSchema, categoriesSchema } from '../entity/Categories';
...
export default async (app: FastifyInstance) => {
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/categories', {
        ...
        // @ts-ignore
        const [items, count] = await getCustomRepository(CategoriesRepository).filter(req.query.q, req.query.page, req.query.limit);
        return {
            rows: items,
            count,
    ...
    const putSchema = TypeBox.Type.Object({
        ...
        data: TypeBox.Type.Partial(newCategoriesSchema),
    });
    app.put<{
        Params: TypeBox.Static<typeof find>,
        ...
        return getCustomRepository(CategoriesRepository).save({
            ...req.body.data,
            id: req.params.id,
        });
    });
